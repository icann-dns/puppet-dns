...
if ENV['BEAKER_TESTMODE'] == 'agent'
  describe 'knot class' do
    context 'test notifies knot -> nsd' do
...
EOS
      ...
      describe command('sed -i \'s/1/2/\' /etc/knot/zone/zone/example.com'), node: dnstop do
        its(:exit_status) { is_expected.to eq 0 }
      end
      describe command('service knot restart'), node: dnstop do
        its(:exit_status) { is_expected.to eq 0 }
